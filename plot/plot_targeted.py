...
baseline_ble_df = pd.read_csv(f'output/data/matched_user_ids_patch.csv')
...
baseline_lte_df = pd.read_csv(f'output/data/matched_user_ids_mob.csv')
...
multi_protocol_df = pd.read_csv(f'output/data/matched_user_ids_rand.csv')
multi_protocol_scores = multi_protocol_df['privacy_score'].values
multi_protocol_scores_sorted = np.sort(multi_protocol_scores)
multi_protocol_users = np.arange(1, len(multi_protocol_scores_sorted) + 1)
count_users_privacy_1 = multi_protocol_df[multi_protocol_df['privacy_score'] == 1.0].shape[0]
...
multi_protocol_df = pd.read_csv(f'output/data/matched_user_ids_baseline.csv')
multi_protocol_scores = multi_protocol_df['privacy_score'].values
multi_protocol_scores_sorted = np.sort(multi_protocol_scores)
multi_protocol_users = np.arange(1, len(multi_protocol_scores_sorted) + 1)
count_users_privacy_1 = multi_protocol_df[multi_protocol_df['privacy_score'] == 1.0].shape[0]
...
xticks=[]
for i in range(0,NUM_USERS+1,10):
    if i==0:
        xticks.append(1)
    else:
        xticks.append(i)
...
#plt.subplots_adjust(left=0, right=1, bottom=0, top=1)
plt.savefig(f'output/images/privacy_leakage_targeted_ccs.pdf', dpi=600, bbox_inches='tight')
plt.show()
